...
//string[] arrayInput = {"hello", "224", "world"}; // - примеры для тестирования программы БЕЗ метода GetArrayStringRandom()
// string[] arrayInput = {"5612", "1247", "-3", "computer_science"};
// string[] arrayInput = {"Russia", "Germany", "Tula"};
...
string[] GetArrayStringRandom() //МЕТОД СОЗДАНИЯ МАССИВА СЛУЧАЙНОГО КОЛИЧЕСТВА СТРОК СЛУЧАЙНОЙ ДЛИНЫ ИЗ СЛУЧАЙНЫХ СИМВОЛОВ
{
    ...
    {
        string symbols = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcsdfsdfdefghijklmnopqrstuvwxyz0123456789!#$%&'()*+,-\"./:;<=>?@[]^_'{|}~";
